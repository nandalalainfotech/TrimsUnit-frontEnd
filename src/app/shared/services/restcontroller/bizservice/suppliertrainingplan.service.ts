...
export class SupplierTrainingPlanManager extends BaseService {
    ...
    NotificationAll() {
        return this.getCallService(`${this.supTrainingPlanUrl}` + "/NotificationAll");
    }

    findAllBySupplierId(supreglNo: any) {
        let supregslno: any = {};
        supregslno['supregslno'] = supreglNo;
        return this.getCallService(`${this.supTrainingPlanUrl}` + "/findAllBySupplierId", supregslno);
    }
